...
/** 모든 Notion API 호출에 적용하는 캐시 설정
 * - dev: 0 (캐시 없음 — Notion 변경이 즉시 반영)
 * - prod: 3600 (1시간)
 */
...
async function fetchChildMarkdown(blockId: string, indent: string = "  "): Promise<string> {
...
function isEndOfList(blocks: BlockObjectResponse[], index: number, type: string): boolean {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    switch (block.type) {
      ...
      case "code": {
        ...
        lines.push(`\`\`\`${lang}`, richTextToMarkdown(block.code.rich_text), "```", "");
        break;
      }
      ...
      case "image": {
        ...
        const caption =
          block.image.caption.length > 0 ? richTextToMarkdown(block.image.caption) : "";
      ...
      case "table": {
        ...
        const tableRows = rows.filter(
          (r): r is BlockObjectResponse & {type: "table_row"} => r.type === "table_row",
        );

        for (let rowIdx = 0; rowIdx < tableRows.length; rowIdx++) {
          const cells = tableRows[rowIdx].table_row.cells.map((cell) => richTextToMarkdown(cell));
...
export const fetchAllProjects = cache(async (): Promise<Project[]> => {
  ...
  do {
    const response = await queryDatabase({
      filter: baseFilter,
      // 최신 연도 우선 → featured(진행중) 프로젝트가 가장 최근 작업이므로 상단 선택에 유리
      sorts: [{property: PROP.year, direction: "descending"}],
      start_cursor: cursor,
      page_size: 100,
    });
...
export const fetchProjectBySlug = cache(
  async (slug: string): Promise<(Project & {content: string}) | null> => {
    const response = await queryDatabase({
      filter: {
        and: [...baseFilter.and, {property: PROP.slug, rich_text: {equals: slug}}],
      },
      page_size: 1,
    });
